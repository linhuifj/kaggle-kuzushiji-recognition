...
def warp_image(img, random_state=None, **kwargs):
    ...
    borderval = random_state.normal(-10,30)
    if borderval < 0:
        borderval = 255
    warped = cv2.remap(img, map_x, map_y, INTERPOLATION[interpolation_method], borderValue=(borderval, borderval, borderval))

    return warped


class DistortAug(object):
    def __init__(self):
        pass
